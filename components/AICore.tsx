...
import { 
  ...
  ChevronRight, ChevronDown, File, Activity, GitCommit, FileDiff
} from 'lucide-react';
...
export const AICore: React.FC = () => {
  ...
  const confirmSave = async () => {
      ...
      alert('Arquivo salvo e Snapshot criado em .backups/');
  };
          ...
          <div className="h-full p-8 overflow-y-auto">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
                   ...
                   <button onClick={executeBuild} disabled={isBuilding} className="w-full bg-primary-600 hover:bg-primary-700 text-white py-2 rounded font-bold">
                     {isBuilding ? 'Compilando...' : 'Executar Build (npm run build)'}
                   </button>
                </div>
                <div className="bg-black border border-slate-800 p-4 rounded-xl font-mono text-xs text-green-400 overflow-y-auto max-h-48">
                   <pre>{terminalOutput || 'Terminal pronto...'}</pre>
                </div>
             </div>
        ...
        {activeTab === 'ENGINEERING' && (
           <div className="flex h-full">
              ...
              <div className="flex-1 flex flex-col bg-slate-950">
                 ...
                 <div className="flex-1 overflow-hidden relative">
                    {ideViewMode === 'CODE' ? (
                       <textarea className="w-full h-full bg-[#0d1117] text-slate-300 font-mono text-sm p-4 resize-none focus:outline-none" value={fileContent} onChange={(e) => setFileContent(e.target.value)} spellCheck={false} />
                        ...
                        <div className="w-full h-full bg-[#0d1117] p-4 font-mono text-xs overflow-auto flex gap-4">
                            <div className="flex-1 border-r border-slate-800 pr-2">
                                <div className="text-red-400 mb-2 font-bold opacity-50">ORIGINAL</div>
                                <pre className="text-slate-500">{originalContent}</pre>
                            </div>
                            <div className="flex-1 pl-2">
                                <div className="text-green-400 mb-2 font-bold opacity-50">MODIFICADO</div>
                                <pre className="text-green-100">{fileContent}</pre>
                            </div>
                        </div>
                    ) : null}
                 </div>
                 ...
                 {selectedFile && (
                    <div className="h-10 border-t border-slate-800 flex items-center justify-end px-4 gap-2 bg-slate-900">
                       <button onClick={handleSaveWithRiskCheck} className="text-xs bg-primary-600 hover:bg-primary-700 text-white px-3 py-1.5 rounded flex items-center gap-1">
              ...
              <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
                  <div className="p-6 border-b border-slate-800 flex justify-between">
                      <h3 className="text-xl font-bold text-white flex items-center gap-2">
                          <GitCommit /> Revisão de Alterações
                      </h3>
                      {riskAnalysis && (
                          <div className={`px-3 py-1 rounded border font-bold ${riskAnalysis.riskLevel === 'HIGH' ? 'bg-red-900/50 border-red-500 text-red-400' : 'bg-green-900/50 border-green-500 text-green-400'}`}>
                              RISCO: {riskAnalysis.riskLevel}
                          </div>
                      )}
                  </div>
                      ...
                      {riskAnalysis && (
                          <div className="grid grid-cols-3 gap-4 text-center">
                              <div className="bg-slate-950 p-4 rounded border border-slate-800">
                                  <div className="text-xs text-slate-500 uppercase">Segurança</div>
                                  <div className="text-2xl font-bold text-blue-400">{riskAnalysis.securityScore}/100</div>
                              </div>
                              <div className="bg-slate-950 p-4 rounded border border-slate-800">
                                  <div className="text-xs text-slate-500 uppercase">Performance</div>
                                  <div className="text-2xl font-bold text-yellow-400">{riskAnalysis.performanceScore}/100</div>
                              </div>
                              <div className="bg-slate-950 p-4 rounded border border-slate-800">
                                  <div className="text-xs text-slate-500 uppercase">Integridade</div>
                                  <div className="text-2xl font-bold text-green-400">{riskAnalysis.integrityScore}/100</div>
                              </div>
                              <div className="col-span-3 text-left text-sm text-slate-300 bg-slate-800 p-3 rounded">
                                  {riskAnalysis.analysis}
                              </div>
                          </div>
                      )}

                      <div className="h-64 bg-black border border-slate-800 rounded p-4 overflow-auto font-mono text-xs flex gap-4">
                            <div className="flex-1 border-r border-slate-800">
                                <div className="text-red-500 mb-1 opacity-50">ORIGINAL</div>
                                <pre className="text-slate-500">{originalContent}</pre>
                            </div>
                            <div className="flex-1">
                                <div className="text-green-500 mb-1 opacity-50">NOVO</div>
                                <pre className="text-green-200">{fileContent}</pre>
                            </div>
                      </div>
                  </div>

                  <div className="p-6 border-t border-slate-800 flex justify-end gap-4">
                      <button onClick={() => setShowDiffModal(false)} className="px-4 py-2 text-slate-400 hover:text-white">Cancelar</button>
                      <button onClick={confirmSave} className="bg-primary-600 hover:bg-primary-700 text-white px-6 py-2 rounded font-bold shadow-[0_0_15px_rgba(37,99,235,0.4)]">
                          Confirmar & Salvar (Snapshot)
                      </button>
                  </div>
              </div>
          </div>
